...
class AnalysisDailyByCity(Resource):
    def get(self):
        try:
            ...
            format_date = datetime.strptime(str(request.args['end_date']), "%Y-%m-%d")
            end_date = format_date + timedelta(days = 1)
...
class AnalysisMonthlyByCity(Resource):
    def get(self):
        try:
            ...
            format_date = datetime.strptime(str(request.args['end_date']), "%Y-%m")
            end_date = format_date + timedelta(days = getDayMonths(format_date))
